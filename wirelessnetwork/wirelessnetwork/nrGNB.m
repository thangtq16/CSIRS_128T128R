    ...
    methods
        function obj = nrGNB(varargin)
            ...
            for idx=1:numel(obj) % For each gNB
                ...
                gNB.MACEntity = nr5g.internal.nrGNBMAC(macInfo, ...
                ...
                % for data scheduling
                gNB.SRSReservedResource = nr5g.internal.nrConfigureSRSReservedResource(gNB);
                ...
                if strcmp(gNB.PHYAbstractionMethod, "none")
                    gNB.PhyEntity = nr5g.internal.nrGNBFullPHY(phyInfo, @(varargin) gnbWeakRef.Handle.processEvents(varargin{:})); % Full PHY
                    gNB.PHYAbstraction = 0;
                else
                    gNB.PhyEntity = nr5g.internal.nrGNBAbstractPHY(phyInfo, @(varargin) gnbWeakRef.Handle.processEvents(varargin{:})); % Abstract PHY
        ...
        function connectUE(obj, UE, varargin)
            ...
            for i=1:numUEs
                ...
                srsConfig = nr5g.internal.nrCreateSRSConfiguration(obj,UE(i),rnti);
        ...
        function configureScheduler(obj, varargin)
            ...
            for idx=1:2:nargin-1
                ...
                if name == "MUMIMOConfigDL"
                    ...
                    schedulerInfo.MUMIMOConfigDL = nr5g.internal.nrNodeValidation.validateConfigureSchedulerMUMIMOInputs(obj, varargin{idx+1}, csiMeasurementSignalDL, mumimoConfigDLCSIRS, mumimoConfigDLSRS);
                    [obj.MUMIMOEnabled] = deal(true);
                elseif name == "LinkAdaptationConfigDL"
                    schedulerInfo.LinkAdaptationConfigDL = nr5g.internal.nrNodeValidation.validateConfigureSchedulerLAInputs(varargin{idx+1}, defaultLAConfigDL);
                elseif name == "LinkAdaptationConfigUL"
                    schedulerInfo.LinkAdaptationConfigUL = nr5g.internal.nrNodeValidation.validateConfigureSchedulerLAInputs(varargin{idx+1}, defaultLAConfigUL);
                elseif name == "CSIMeasurementSignalDL"
                    schedulerInfo.CSIMeasurementSignalDL = nr5g.internal.nrNodeValidation.validateConfigureSchedulerCSIMeasurementSignalDL(obj, varargin{idx+1});
                else
