function [PMISet,info] = nrPMIReport(carrier,csirs,reportConfig,nLayers,H,nVar)
%nrPMIReport PDSCH precoding matrix indicator calculation
%   [PMISET,INFO] = nrPMIReport(CARRIER,CSIRS,REPORTCONFIG,NLAYERS,H,NVAR)
...
%   CARRIER, CSI-RS configuration CSIRS, CSI-RS report configuration object
%   REPORTCONFIG, number of transmission layers NLAYERS, estimated channel
%   information H, and estimated inter-cell interference pluse noise
%   variance or noise variance at the receiver NVAR.
%
...
%   Copyright 2024 The MathWorks, Inc.
    ...
    subbandInfo = nr5g.internal.getPMISubbandInfo(carrier,reportConfig);
    ...
    csirsIndSubs = nr5g.internal.getCSIRSIndicesForCSI(carrier,csirs);
    [H_bwp,csirsIndBWP_k,csirsIndBWP_l,csirsIndBWP_p] = getCSIRSIndicesAndHInBWP(carrier,reportConfig,H,csirsIndSubs);    
    if(~isscalar(nVar))
        nVar = getCSIRSIndicesAndHInBWP(carrier,reportConfig,nVar,csirsIndSubs);
    end

    % Generate PMI set and output information structure with NaNs
    [PMINaNSet,nanInfo] = getPMINaNSet(reportConfig,subbandInfo,codebook,indexSetSizes,numCSIRSPorts,nLayers,csirsIndBWP_k);
    ...
    % Interference plus noise covariance matrices at CSI-RS locations
    if(~isscalar(nVar))
        nVarTemp = reshape(nVar,size(nVar,1),size(nVar,2),[]);
        nVarUpdated = nVarTemp(:,:,csirsIndBWP_k+(csirsIndBWP_l-1)*size(nVar,3)); % Covariance matrices at CSI-RS locations
        nVar = nVarUpdated;
    end
...
%   matrices, as defined in TS 38.214 Tables 5.2.2.2.1-1 to 5.2.2.2.1-12
%
%   REPORTCONFIG is CSI-RS report configuration object nrCSIReportConfig
%
...
%   returns the CSI-RS indices in subscript form with in the BWP
    ...
    H_bwp = H(:,:,bwpStart*12 + 1: (bwpStart + nSizeBWP)*12,:); % w.r.t. BWP
    ...
    indInBWP = (csirsIndBWP_kTemp >= bwpStart*12 + 1) & csirsIndBWP_kTemp <= (bwpStart + nSizeBWP)*12;
...
function [PMINaNSet,nanInfo] = getPMINaNSet(reportConfig,subbandInfo,codebook,indexSetSizes,numCSIRSPorts,nLayers,csirsIndBWP_k)
%   [PMINaNSet,nanInfo] = getPMINaNSet(reportConfig,subbandInfo,codebook,indexSetSizes,numCSIRSPorts,nLayers)
...
function [PMISet,W,SINRPerREPMI,SubbandSINRs] = getTypeIPMISubband(reportConfig,codebook,PMISet,SINRPerRE,subbandInfo,indexSetSizes,csirsIndSubs_k)
%   [PMISet,W,SINRPerREPMI,SubbandSINRs] = getTypeIPMISubband(reportConfig,codebook,PMISet,SINRPerRE,subbandInfo,indexSetSizes,csirsIndSubs_k)
...
%   and enhanced type II codebooks
    ...
    if ~isType2
        configTable = reportConfig.Tables.EnhancedType2Configurations;
        beta = configTable{reportConfig.ParameterCombination,5};
    end
    ...
    if numel(reportConfig.CodebookSubsetRestriction)
        ...
        for q1Val = 1:O1
            for q2Val = 1:O2
                ...
                if ~(any(isnan(WTemp),'all') || any(isinf(WTemp),'all'))
                    SINRValues(:,:,i12Val,q1Val,q2Val) = nr5g.internal.nrPostEqualizedSINR(H,WTemp,nVar);
                end
            end
        end
    end
...
%   information
    ...
    if isType2
        ...
        configTable = reportConfig.Tables.EnhancedType2Configurations;
        pv = configTable{reportConfig.ParameterCombination,3+floor((nLayers-1)/2)};
        beta = configTable{reportConfig.ParameterCombination,5};
        % Get the number of DFT vectors used for DFT compression
        Mv = ceil(pv*N3/reportConfig.NumberOfPMISubbandsPerCQISubband);
...
%   quantizes the W2 matrix in case of enhanced type II codebooks
    ...
    if maxAmpPol0 ~= 0 % This is to avoid NaNs after normalization
        normalizedAmpPol0 = normalizedAmpPol0./maxAmpPol0; % It is of size numBeams-by-Mv-by-nLayers, for first polarization
    end
    if maxAmpPol1 ~= 0 % This is to avoid NaNs after normalization
...
%   the layers
    ...
    % Sort amplitudes and null the weakest 2*numBeams*Mv*nLayers-K0 or
    % 2*numBeams*Mv*nLayers-2*K0 elements in p2, k2, c, and phase.
    ...
    nullIndices = idx(1:totCoeff-((1+(nLayers>1))*K0));
...
%   computes SINR per RE level for all the precoding matrices
    ...
    if(csirsIndSubs_length < prod(indexSetSizes)) % Consider precoding matrix as page matrix
        for reIdx = 1:csirsIndSubs_length
            ...
            sinr = nr5g.internal.nrPostEqualizedSINR(Htemp,codebook,nVarTemp);
        ...
        for Indx = 1:prod(indexSetSizes)
            if any(codebook(:,:,Indx),'all')
                ...
                SINRPerRE(:,:,Indx) = nr5g.internal.nrPostEqualizedSINR(Hcsirs,codebook(:,:,Indx),nVar);
            end
        end
    end
end
